...
2. 复制环境变量模板并填写最少必需项：
   ...
   至少补齐：`OCTORILL_ENCRYPTION_KEY_BASE64`、`GITHUB_CLIENT_ID`、`GITHUB_CLIENT_SECRET`、`GITHUB_OAUTH_REDIRECT_URL`。如果要测试 LinuxDO 绑定，再额外填写 `LINUXDO_CLIENT_ID`、`LINUXDO_CLIENT_SECRET`、`LINUXDO_OAUTH_REDIRECT_URL`，三项必须同时出现。
