...
- 活动图在 `>=1024px`、`640-1023px` 和 `<640px` 分别展示最近 `50`、`36` 和 `25` 个小时桶；键盘左右漫游只在当前可见窗口内进行。
- 网格在所有断点均以 `width: 100%` 与自适应列宽渲染，不产生页面或网格横向滚动。单元格保持正方形，最大尺寸依次为 `12px`、`11px` 和 `9px`，相邻单元不得重叠。
...
- Given 管理员首次打开 LLM 调度页
  When 活动数据加载完成
  Then 默认显示逐模型活动图；`>=1024px` 显示最近 50 小时、`640-1023px` 显示 36 小时、`<640px` 显示 25 小时，并可切回原有模型卡片。

- Given 任一活动图断点
  When 计算网格布局
  Then 页面与网格均没有横向溢出，相邻单元不重叠；窄屏模型名只出现在网格下方图例。
...
- 管理端活动图桌面深色聚合窗、`393x852` 移动端浅色 25 小时紧凑页面，以及 Storybook 聚合窗组件证据。
