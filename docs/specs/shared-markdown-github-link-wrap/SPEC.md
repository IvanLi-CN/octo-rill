# 共享 Markdown GitHub 链接短标签与自动换行防裁剪
...
- [Release 日报内容格式 V2 与历史快照修复](../release-daily-brief-v2/SPEC.md)已经在后端 canonical brief 里收敛 related links，但前端 shared Markdown 仍需要补一层“只压缩裸 GitHub autolink、保持已有自定义标签不变”的展示守卫，避免历史数据或其它 Markdown 来源继续泄漏超长链接。
...
- `cd web && bun run e2e -- release-detail.spec.ts --grep "shared markdown compacts raw GitHub links and keeps long links wrapped"`
