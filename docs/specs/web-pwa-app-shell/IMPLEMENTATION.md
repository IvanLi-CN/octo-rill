...
- Version notice now surfaces both Service Worker refresh and native install prompt actions in one shared shell surface, with content-driven mobile wrapping, right-aligned actions, and preserved visual button dimensions.
...
- Worker activation resolves the current waiting Worker at click time, waits for one controller change before reloading, exposes retry after a 15-second timeout, and keeps the `SKIP_WAITING` message event alive with `waitUntil`.
...
- Visual evidence captures the authenticated Dashboard update notice states at 375px and 320px browser viewports, plus the Landing offline boot fallback and authenticated Dashboard offline states from mock-only sources.
