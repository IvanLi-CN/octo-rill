...
- 翻译、智能摘要、日报三条链路的数据分散在实时任务列表里，不适合做按天趋势观察与异常定位。
...
- 使用第三方图表库展示翻译、智能摘要、日报三类任务的今日执行情况与 7 天 / 30 天趋势。
...
- SQLite 新增 `admin_dashboard_daily_rollups` 作为每日聚合真相源。
- 后端新增 `GET /api/admin/dashboard?window=7d|30d`，返回 KPI、今日实时快照、状态分布、任务占比、趋势点位与窗口元数据。
...
- 必须展示翻译、智能摘要、日报三类任务的今日 Stats、今日占比与趋势图表。
- 图表必须使用第三方库实现，并可在 Storybook 中稳定渲染。
- 历史趋势必须来自每日 rollup；今日卡片和进行中任务必须由实时查询覆盖，不依赖 rollup 刷新完成。
- 统计口径必须覆盖 `queued / running / succeeded / failed / canceled`。
...
1. **管理员进入仪表盘**
   - 管理员访问 `/admin`。
   - 页面展示 Hero 区、4 个 KPI 卡片、进行中链路摘要、图表区与高密度 Stats 表。
...
3. **今日实时覆盖**
   - 仪表盘请求实时查询今日用户活跃、今日任务状态、今日任务占比与进行中任务。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `GET /api/admin/dashboard` | HTTP API | external | New | `./contracts/http-apis.md` | backend | web-admin | 返回窗口化 KPI、今日实时快照、状态分布与趋势点位 |
...
- Given 仪表盘页面
  When 查看可视化区域
  Then 至少包含今日状态分布、今日任务占比、趋势图与活跃用户/规模对比图。
...
- Given scheduler 重复刷新最近 30 天 rollup
  When 多次执行 upsert
  Then `admin_dashboard_daily_rollups` 中相同 `rollup_date + time_zone + task_type` 仍只有一条最新记录。

- Given 今日 rollup 尚未刷新完成
  When 请求 `GET /api/admin/dashboard`
  Then 今日 KPI、进行中任务与今日趋势点仍返回实时值。
...
- 今日数据使用实时查询覆盖，包括今日活跃、进行中任务、今日状态分布与趋势中的“今天”点。
...
- 假设：本期只覆盖翻译、智能摘要、日报三类核心任务，已满足 v1 运营总览需求。
