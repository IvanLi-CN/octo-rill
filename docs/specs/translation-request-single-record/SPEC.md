...
本主题记录已经交付的 `0019` 单条请求重建。它的用户范围、旧请求表和破坏式迁移只解释当时的实现事实；当前和未来的内容处理身份、请求关联、结果投影与迁移规则由[全局翻译与润色工作模型](../global-translation-and-polish/SPEC.md)取代。本主题中的破坏式迁移不得作为后续演进方案复用。
...
- Given 调用方以 `mode=wait` 或 `mode=stream` 提交单个翻译项
  When 请求完成，或 `wait` 调用达到 `max_wait_ms` 预算
  Then `GET` 与 stream 事件都只返回单个 `result`；`wait` 在预算耗尽时返回当前单结果快照，不存在 `items[]`、`batch_ids[]`、`completed_item_count/item_count`。
...
- `cd web && bun run e2e -- e2e/admin-jobs.spec.ts e2e/release-detail.spec.ts`
