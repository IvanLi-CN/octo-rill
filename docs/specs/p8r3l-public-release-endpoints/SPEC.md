...
- 已认证的公开 Release 列表可调用既有反应接口：
  ...
  - 启用后调用 `POST /api/feed/reactions/refresh` 批量读取当前列表反应，每批最多 100 个 `release_id`；只有响应中返回的 Release 才显示按钮，并通过 `POST /api/release/reactions/toggle` 更新单条记录。列表分页、补 gap 或高亮窗口在请求途中改变时，已发出的有效响应仍按 `release_id` 合并，不能因列表更新而永久丢失控件；非 PAT 的瞬态批次失败独立重试至多三次，且不丢弃同轮已成功的批次。
