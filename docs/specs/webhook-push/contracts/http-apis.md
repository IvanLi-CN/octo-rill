...
- `PATCH /api/me/webhook-push`：请求体 `{ "desired_state": "enabled" | "paused" | "deleted" }`；启用时校验前置条件，所有远端动作异步排队。
- `POST /api/me/webhook-push/reconcile`：请求体可选 `{ "repo_id": number }`；由用户手动发起检查并修复全量或单仓。用户已有未终态操作时返回 `409 webhook_push_operation_in_progress`。
