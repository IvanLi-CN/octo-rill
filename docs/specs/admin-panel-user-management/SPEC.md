...
2. **管理员查看用户列表**
   - 管理员从 Dashboard 账号菜单进入独立管理面板，再访问 `/admin/users` 用户管理页。
...
- 列表中的“有效关注仓库数”只读取本地已同步仓库快照，不在管理员列表请求里引入新的 GitHub 在线刷新副作用；更完整的 repo budget / 老化治理由 [repo-refresh-governance](../repo-refresh-governance/SPEC.md) 负责。
