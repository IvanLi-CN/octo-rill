...
- Do not hold the writer permit around GitHub API, AI calls, HTTP calls, or long computation. Prepare data first, including notification-thread refreshes or other repair lookups, then acquire the permit, write quickly, and release it. If the repair writes back into rows that other sync paths can update concurrently, re-read the current row inside the writer transaction before applying the repair so stale lookup results do not overwrite fresher fields.
