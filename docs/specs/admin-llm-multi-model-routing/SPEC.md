...
- 桌面端展示接口返回的完整 50 小时窗口；移动端默认收起模型名，仅展示最近 25 小时，并以水平起止时间替代逐列旋转刻度。展开模型名时允许横向滚动。
...
- Given 管理员首次打开 LLM 调度页
  When 活动数据加载完成
  Then 桌面端默认显示最近 50 小时逐模型活动图，移动端紧凑显示最近 25 小时，并可切回原有模型卡片。
...
- 管理端活动图桌面深色聚合窗与 `393x852` 移动端浅色 25 小时紧凑页面证据。
