...
- 本主题复用 `api-keys` 的 Bearer API Key 业务接口鉴权边界，不新增匿名读取面。
- 本主题复用 `public-release-endpoints` 的 public release usage 与异步同步链路，但不回写其匿名 public page 主题边界。
- 本主题扩展 `authenticated-scoped-focus-feed` 的 scoped focus feed 契约，新增 `following` scope 与关注仓库阅读面。
- 当前验证结果：
  - `cargo check` 通过
