...
- 当 `lefthook.yml`、`package.json` 或根目录依赖发生变化时，重新执行 `bun install` 或 `bun run hooks:install`；若变更发生在 linked worktree 内，安装脚本会按当前 worktree 的 `lefthook.yml` 生成共享 wrappers，但仍复用主工作区的 repo-local `lefthook` 二进制。
