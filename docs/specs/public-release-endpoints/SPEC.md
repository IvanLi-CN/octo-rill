...
- 目录必须以最小必要滚动将阅读当前版本保持在可见范围。目录项是可访问的版本链接，选中状态不得只靠颜色表达；键盘焦点、粘性标题或焦点动画不得遮挡可操作项。
- 详情卡片的指针进入、键盘焦点和标题激活都必须把该 Release 作为当前阅读版本，并通过同一协调路径更新目录选中项；目录项尚未挂载或不在可视区域时，按稳定 `release_id` 索引挂载并以最小必要滚动露出它。指针悬浮只同步阅读状态，不改写 URL；标题激活继续使用 SPA 导航并保留该目录聚焦事务。滚轮、按下、键盘、卡片进入/聚焦和标题激活等真实用户事件优先于进行中的程序化事务，必须先取消旧动画/重试再传播当前事件；浏览器重挂载造成的静态指针重排不得覆盖明确的导航目标。
- 当 URL 或目录明确选择一个版本时，详情时间线必须先将目标卡片载入并定位。目标卡片已完整处于可用阅读区域时不得改变当前位置；仅在目标不可见或部分可见时按最近距离滚动至完整展示，不居中、不固定顶对齐，也不强制露出前一张卡片的 `3lh`。
- 每次 URL 或目录触发的程序化详情定位在目标卡片上播放约三秒的克制焦点提示：仅使用不改变布局的轮廓、阴影或透明度过渡，并且一次只提示一个卡片。自然滚动只更新目录选中态，不反复播放卡片动画。上述真实用户输入可中断程序化滚动；`prefers-reduced-motion` 下直接呈现最终焦点状态，不播放动画。
- 既有 `highlight`、`highlight_start`、`highlight_end` 与 `highlight_active` 仍表达高亮上下文和导航进度，不得将版本目录或详情时间线过滤成高亮集合。tag 深链的首载必须取得目标周边的时间线窗口，避免任一栏从第一页逐页探测目标。
...
- 双栏阅读器复用公开 Release 列表的 target-centered window、双向 cursor 和内部 gap 契约；tag 深链不得只读取单条详情后再由任一栏另行分页探测周边 Release。
...
- Given 用户以 `lg` 或更宽视口打开公开 Release 列表或 tag 详情链接
  When 数据就绪
  Then 页面在一个阅读器中显示左侧版本目录和右侧详情时间线；两栏按同一时间倒序 Release 序列对应，但各自只渲染视口与 overscan 项。
...
- Given 用户打开 `/public/:owner/:repo/releases/tag/:tag`
  ...
  Then 目录选中该版本，详情时间线以目标为阅读当前版本；目标已完整可见时当前位置保持不变，否则只按最近距离滚动到完整展示；目标卡片播放一次约三秒的焦点提示。

- Given 用户从版本目录明确选择一个版本
  When 路由与详情时间线更新
  Then 浏览器进入该版本的 public-only tag 链接，详情时间线定位其卡片并播放一次焦点提示；目录与详情不会在定位过程中因中间卡片短暂可见而选择其他版本。

- Given 用户从无 tag 的 `/:owner/:repo/releases` 浏览入口开始阅读
  When 初始最新 Release 载入或详情时间线发生自然滚动
  Then 目录选中阅读当前版本，但页面保持无 tag 的浏览入口 URL。

- Given 用户将指针悬浮在详情时间线中的 Release 卡片上
  When 该卡片已挂载
  Then 该 Release 成为阅读当前版本，目录对应项被按需挂载、最小滚动露出并以 `aria-current` 表达；悬浮不改写 URL。

- Given 用户聚焦详情卡片或点击其标题
  When 目录当前可视区域没有对应项，或对应项尚未被虚拟列表挂载
  Then 目录通过共享的 Release ID 顺序找到并露出对应项；标题点击保持 SPA 路由，不因父级用户交互取消而丢失目录聚焦。

- Given 用户正在显式 tag 链接的详情时间线中自然阅读另一张卡片
  When 可见性规则确定新的阅读当前版本
  Then 目录以最小必要滚动保持该版本可见，当前 tag 使用 replace 更新，且不产生额外的浏览历史项或三秒焦点动画。

- Given 详情时间线中没有完整展示的卡片
  When 一张卡片经过可视区域约 30% 的阅读线
  Then 该卡片成为阅读当前版本；若存在完整展示的卡片，则以最上方完整卡片优先。

- Given 版本目录和详情时间线需要继续加载 older、newer 或内部 gap 数据
  When 任一虚拟视口接近其已加载边界
  Then 页面通过共享的 Release ID 序列扩展对应窗口，保持当前阅读版本和两栏的阅读锚点，不重复从第一页探测目标。

- Given URL 或目录触发程序化详情定位
  When 用户开启减少动态效果，或在定位/动画期间滚轮、按下、聚焦卡片、点击标题或使用键盘
  Then 减少动态效果时直接呈现最终焦点状态；用户输入立即中断未完成的程序化移动，且页面继续保持可操作。
