# Authenticated Scoped Focus Feed
...
- `dashboard-tab-path-release-deep-link` 已经冻结了 Dashboard 顶部 tab 路径化与 release detail deep link 基础契约，`owned-release-opt-in` 已经定义了“我的发布”背后的 owner baseline 语义；本轮需要在它们之上新增一个独立的 scoped 阅读主题，而不是回写旧 spec 边界。
...
- 不把组织仓库纳入 `/focus/mine` 的“个人仓库”范围；`owned-release-opt-in` 继续只定义全局 Release 阅读面的“我的发布”开关。
...
- `docs/specs/owned-release-opt-in/SPEC.md`
- `docs/specs/dashboard-quasi-realtime-updates/SPEC.md`
