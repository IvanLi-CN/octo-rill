...
- REQ-GTP-ADMISSION: `superseded` 是不可重开的终态；重复 source admission 必须幂等并记录独立准入事实。规范来源 adapter 必须把来源更新时间及等价权威 tie-break revision 写入冻结 source snapshot 的非身份元数据；同步到达时间和 `source_hash` 不得作为 source revision。每个 provider 请求前必须在 SQLite writer 事务内验证 live lease、source existence 和 source currentness，并写入 provider-admission 事实；已知旧 source 不得调用 provider，准入拒绝或准入检查异常都不得遗留已落库的 running 调用，准入后源变化的调用不得发布结果或安排旧 source 恢复。启动与 recovery reconciliation 只能无 provider 地收口旧非运行工作。
