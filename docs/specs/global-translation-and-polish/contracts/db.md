...
- Readers resolve the expected source identity first, preferring the current model profile but falling back to the newest ready projection for that same source when routing changes; a model-profile change must not hide published content.
