...
- release 翻译的输入预算默认按模型目录自动推导；如需手动覆盖，请在管理员任务中心 `LLM 调度` 页签的“配置 LLM 运行参数”里填写 `LLM 输入长度上限（tokens）`。
- 管理员在任务中心保存新的 LLM / translation runtime 配置后，这些值会持久化到数据库，并在后续重启时覆盖首次启动的 env/default 或空值种子。
