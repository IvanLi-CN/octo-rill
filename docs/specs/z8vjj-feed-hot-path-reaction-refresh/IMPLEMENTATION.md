...
- 前端使用短 TTL、100-id batch 切分、in-flight refresh toggle guard 与 optimistic pending guard，避免刷新结果造成请求风暴、超限失败、刷新中误操作或覆盖用户正在提交的 reaction 状态；refresh 失败后不会永久阻断 toggle。
