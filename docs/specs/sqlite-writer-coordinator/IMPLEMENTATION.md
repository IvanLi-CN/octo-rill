...
- API key 创建/撤销、daily brief profile 更新、LinuxDO/GitHub connection unlink 与 passkey 删除使用 foreground lane；API key `last_used_at` touch 由单个后台 worker 按 key 合并后排队到 background lane，SQL 只允许时间戳单调前进，在不阻塞认证响应的同时避免无界 task 积压与乱序回退；stale runtime owner prune 使用 best-effort lane，runtime owner unregister 使用 background lane，避免这些应用内写路径绕过共享 coordinator。
