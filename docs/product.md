...
- 默认语言：中文（`zh-CN`）。
- 触发方式：**懒加载自动**。前端会基于真实 DOM 视口几何持续重算 demand window：把当前视口内所有可见 release，以及最后一个可见卡片之后的连续 10 条 release，一起交给结果聚合接口。
...
- 重试约束：自动可见窗口轮询不会把 `error` 条目重新入队；只有用户手动点击“翻译”这类显式重试，才会带上重试意图并把旧错误任务重置回队列。
