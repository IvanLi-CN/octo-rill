...
- Stories to add/update: `web/src/stories/AppBoot.stories.tsx`
- Docs pages / state galleries to add/update: `Pages/App Boot` autodocs
- `play` / interaction coverage to add/update: `Dashboard Warm Skeleton` 对“无 login pill / 无具体 tabs 文案 / 无登录 CTA”的断言
- Visual regression baseline changes (if any): `docs/specs/6x959-dashboard-startup-skeleton-header-tabs-alignment/assets/dashboard-warm-skeleton-top-shell.png`
...
- [x] M3: Storybook 审阅断言、视觉证据与前端校验完成。
...
- Storybook 断言与视觉证据同步更新，避免未来再次回归到“骨架态泄露真实 tabs 文案”的状态。
