...
同一生产结构合成副本的 global-mode 列表读取对 `24h`、`7d`、`30d` 三个窗口分别执行 Release、公告、通知和日报测量；testbox 上各类窗口的最慢 p95/p99 为 Release `157/157ms`、公告 `102/102ms`、通知 `182/182ms`、日报 `178/178ms`。EXPLAIN 断言来源时间/canonical 索引，并确认处理状态由窗口候选 ID 通过 `idx_content_work_items_resource` 点查。列表通过当前页窗口总数返回精确 `total`。
...
| --- | ---: | ---: | ---: | ---: |
| Release | 5,000 | 0.203s | 0.205s | 0.205s |
| 公告 | 2,500 | 0.116s | 0.116s | 0.116s |
| 通知 | 2,500 | 0.119s | 0.121s | 0.121s |
| 日报 | 5,000 | 0.131s | 0.143s | 0.143s |
