...
- 前端 batch 大小必须匹配后端解析上限；分页累积列表不能把所有已渲染 ID 无界塞进单个 refresh 请求，也不能对已刷新或已在 refresh 中的同一 ID 重复发起请求。
...
- 如果缓存默认值可能改变用户操作语义，应优先让 mutation API 做实时校验或采用显式 desired-state 设计兜底；后台 refresh 不应通过额外 UI 阻断首屏或正常操作。
