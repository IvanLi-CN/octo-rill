...
- `BrandLogo` 自动主题逻辑、共享 footer 主题入口、Landing/Notice 等关键浅色硬编码面的暗色适配。
...
- Landing、Dashboard、Admin、共享 notice/footer 在暗色下必须保持可读与可辨识层级。
...
- 用户可通过共享 footer 中的主题入口切换为浅色、深色或跟随系统；显式偏好写入浏览器本地，刷新后保持一致。
...
| `ThemeToggle` | internal UI component | internal | New | None | web | App footer / stories | 唯一共享入口 |
...
- Given 用户在 footer 中切换为 `浅色`、`深色` 或 `跟随系统`，When 当前页更新并刷新，Then 主题表现与用户选择一致。
...
- Stories to add/update: Landing、App shell、App meta footer、必要的主题组件 stories。
- Docs pages / state galleries to add/update: Landing / footer / shell 的稳定审阅面。
...
- [x] M2: 完成 BrandLogo、Landing、共享 notice/footer 的暗色适配
...
- 2026-04-11: 新建规格，冻结三态主题、本地持久化、共享 footer 入口与 Storybook 视觉验收口径。
- 2026-04-11: 接入启动前主题脚本、`ThemeProvider / useTheme / ThemeToggle`、BrandLogo 跟随应用已解析主题，并完成 Landing / Dashboard / Admin 共享表面的暗色适配。
- 2026-04-11: 刷新 Storybook 主题 toolbar、Landing / Footer / ThemeToggle 审阅面与 Playwright 主题回归覆盖，沉淀最终视觉证据并完成 review-loop 清空。
