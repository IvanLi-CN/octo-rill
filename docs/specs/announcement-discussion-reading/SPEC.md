# 公告翻译/润色与 Discussion 详情页对齐

## 背景 / 问题陈述

- `dashboard-social-activity` 已把公告作为 ambient feed 事件接入 `全部` tab，但当前公告仍停留在“只读内容卡 + GitHub 外链”阶段，没有对齐 release 卡片的 `原文 / 翻译 / 润色` 阅读模型。
- `release-feed-smart-tabs` 已冻结 lane selector、自动触发、错误重试与原文回退语义，但这套 contract 目前只覆盖 release，没有扩展到 announcement。
- `dashboard-tab-path-release-deep-link` 已冻结 release canonical deep link 与 `from` 返回语义，但 discussion 公告仍缺少站内 canonical path、detail API 与 scope-aware round-trip。
...
- `docs/specs/release-feed-smart-tabs/SPEC.md`
- `docs/specs/dashboard-tab-path-release-deep-link/SPEC.md`
- `docs/specs/authenticated-scoped-focus-feed/SPEC.md`
- `docs/product.md`
