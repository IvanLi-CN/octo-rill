...
- `control ref`: 受控验收开始前解析并冻结的 `main` 不可变提交引用。
- `candidate ref`: control 的受限严格后继；候选与 control 的差异只能落在本主题声明的 CI、报告/摘要、验收、合同测试和主题文档路径。
...
- CI MUST emit both list output and `test-results/playwright-results.json`; an `always()` follow-up MUST write `test-results/playwright-summary.json` with total, final passed/failed/skipped, flaky, and retry counts.
...
- The acceptance driver MUST make performance claims only from two refs whose immutable SHAs, strict control-to-candidate ancestry, allowed file delta, run attempts, ordering, required jobs, Frontend E2E job timestamps, and Playwright result artifacts are validated.
...
- Pass condition: immutable SHA/ancestry checks, restricted delta, ten-pair alternating serial order, run-attempt/job validation, E2E artifact parsing, test-count parity, retry/final-failure thresholds, statistics, and fail-closed cases are exercised without network access.
