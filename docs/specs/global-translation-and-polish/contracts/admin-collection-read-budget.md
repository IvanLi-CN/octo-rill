...
同一生产结构合成副本的列表读取对 `24h`、`7d`、`30d` 三个窗口分别执行 Release、公告、通知和日报测量；testbox 上各类窗口的最慢 p95/p99 为日报 `111/111ms`、Release `94/94ms`、公告 `47/47ms`、通知 `46/46ms`。每次列表读取只在来源窗口内聚合处理事实，并通过当前页窗口总数返回精确 `total`。
