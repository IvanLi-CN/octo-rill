...
- 网格在所有断点均以 `width: 100%` 与自适应列宽渲染，不产生页面或网格横向滚动。单元格保持正方形，相邻单元不得重叠；最末时间刻度从右边界向内展开，不得额外造成溢出。
...
- 管理端活动图桌面深色完整 50 桶页面、`393x852` 移动端浅色按容器容量紧凑页面，以及 Storybook 37 桶聚合窗组件证据。
