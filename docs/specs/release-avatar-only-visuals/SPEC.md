# Release 视图固定 owner/org avatar

## 背景 / 问题陈述

Release Feed 与 Release 详情曾采用 `custom social preview -> owner/org avatar -> text-only` 的仓库视觉识别顺序。实际阅读体验里，social preview 横图在小尺寸下噪音偏高，也会让仓库来源锚点不够稳定。
...
- `cd web && bunx playwright test e2e/release-detail.spec.ts`
