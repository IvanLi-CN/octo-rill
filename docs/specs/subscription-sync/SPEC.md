...
- 不在本 spec 内继续扩展 repo release 管理后台；独立“仓库治理”页与 budget 调度合同已由 [repo-refresh-governance](../repo-refresh-governance/SPEC.md) 接管。
...
- 新增或演进专门的 repo release 管理后台页面；该方向现由 [repo-refresh-governance](../repo-refresh-governance/SPEC.md) 负责。
...
- 若任务在 `star_refreshed` 前硬失败，必须额外写入 `task.progress(stage=star_failed)` 诊断事件，并把失败阶段、耗时、timeout/http 状态或错误链写入 task log，便于管理员区分 `timeout / dns / connect / tls / request / response / decode / http_status / local`。
