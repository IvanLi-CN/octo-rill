...
- Dashboard 弹层与 `/settings` 内的 GitHub PAT 输入框都必须在默认隐藏态使用原生 `password` / secure-text 语义，设置 `autocomplete="off"`，并补充密码管理器忽略提示，避免被当成当前站点登录密码框自动填充或触发生成密码建议；仅在用户显式点亮后切到明文。隐藏态还必须继续保留在辅助功能树中的可聚焦、可编辑语义，辅助技术读取到的值必须保持为掩码而不是明文 token，并提供额外提示说明当前处于隐藏编辑态。
...
- 2026-04-22：将 GitHub PAT 输入契约调整为默认保持原生 `password` / secure-text + `autocomplete="off"`、显式点亮后再切到明文，并补充密码管理器忽略提示，同时补齐隐藏态对辅助功能树的掩码保护、切回隐藏态后的焦点回收以及自动填充回归覆盖。
