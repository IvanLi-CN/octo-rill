# Dashboard 日报/详情卡片自适应高度修正（#r8m4k）
...
- Dashboard `日报` tab 中，`Release 日报` 卡片把 Markdown 正文包在固定高度容器里，长内容会在卡片内部出现纵向滚动条。
- 从日报内部链接打开 `Release 详情` 后，详情正文也沿用同样的固定高度策略，形成第二层嵌套滚动区。
- 这类嵌套滚动会破坏页面阅读流，尤其在日报内容跨多个项目时，用户必须分别滚动页面和卡片，阅读体验不自然。
...
- 移除 `Release 详情` 卡片正文内部的固定最大高度与纵向滚动限制。
...
- `Release 详情` 正文容器不得再设置固定最大高度。
- `Release 详情` 正文容器不得再启用纵向内部滚动。
- 页面长内容阅读必须由页面主滚动承担。
...
- 用户点击日报中的内部 release 链接后，`Release 详情` 卡片加载长正文并继续随内容增高，不出现第二层纵向滚动区。
...
- Given 用户从日报内部打开一条长 release 详情
  When 详情正文渲染完成
  Then `Release 详情` 卡片正文区域不再包含 `max-h-96` / `overflow-auto` 风格约束，且主页面滚动可覆盖完整内容。

- Given Storybook 长内容场景
  When 进入长日报与长详情 story
  Then 可以稳定复现“卡片随内容扩展、无内部纵向滚动”的状态。
...
![Brief detail card grows with long content](./assets/briefs-long-detail.png)
...
- 保留现有卡片边框、内边距和 Markdown 样式，仅移除正文容器的固定高度与纵向滚动限制。
- 在 Dashboard Storybook 中加入长日报与长详情场景，用 mock 数据稳定复现长内容布局。
- 用 Storybook 作为视觉证据源，验证页面整体滚动替代卡片内部滚动。

## 风险 / 开放问题 / 假设（Risks, Open Questions, Assumptions）

- 风险：若 Storybook 长详情 mock 未正确拦截详情接口，story 会退回加载失败态，影响视觉验证。
- 开放问题：无。
