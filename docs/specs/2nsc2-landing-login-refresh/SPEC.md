...
- 首屏主标题固定为 `把和你有关的 GitHub 动态放到一个首页里`。
- 首屏说明固定为 `这里集中看 Releases、被加星、被关注和 Inbox；Release 默认提供中文翻译。`
- 首屏卖点压缩为 3 条短句：`Releases 信息流`、`被加星 / 被关注`、`日报 + Inbox 入口`。
...
- 桌面宽度下，Landing 采用左侧品牌说明区、右侧登录卡的双栏结构；品牌区只保留 wordmark、标题、短说明和 3 条短卖点。
...
- Given 未登录访问 `/`
  When Landing 渲染完成
  Then 页面展示桌面双栏 / 移动端 CTA 优先单栏的新结构，且主标题为 `把和你有关的 GitHub 动态放到一个首页里`。
...
- Given Landing 正常渲染
  When 检查首屏文案
  Then 页面只保留 1 个主标题、1 段短说明和 3 条短卖点，不再出现 `Start here`、`为 GitHub Release 阅读而生`、`使用 GitHub 登录` 等旧口径。
...
- 桌面默认态：Landing 已收敛为更干净的双栏结构，左侧只保留品牌说明与 3 条短卖点，右侧只保留简洁登录卡与唯一主 CTA。

![Landing 桌面默认态（暗色）](./assets/landing-desktop-dark.png)

- 移动端首屏：登录卡已经前置到品牌说明之前，`连接到 GitHub` 在首屏内完整可见，不需要向下滚动寻找按钮。
...
- [x] M2: Storybook 默认态、错误态与移动端审阅入口更新完成。
...
- 需要决策的问题：None。
- 假设（需主人确认）：Landing 首屏优先强调“和我有关的 GitHub 动态首页”，只在 Release 语境下提中文翻译，不把 AI 单独当主卖点。

## 变更记录（Change log）

- 2026-04-12: 新建规格，冻结 Landing 登录页重做的文案、布局、移动端 CTA 与视觉证据口径。
- 2026-04-12: 完成 Landing 页面、Storybook 和 Playwright 本地收口，并补入桌面/移动端视觉证据。
- 2026-04-12: 根据 README 与 `docs/product.md`，把首屏文案改写为“和我有关的 GitHub 动态首页”口径，明确覆盖 Releases、社交动态、日报与 Inbox。
