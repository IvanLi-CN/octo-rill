...
### Tabs：全部 / Releases / 被加星 / 被关注 / 日报 / Inbox

页面整体采用**固定左右两栏**布局：

- **左侧主列**：根据 Tab 展示对应内容（聚合 Feed / Release Feed / 社交动态 / 日报 / Inbox 列表）。
- **右侧侧栏**：展示“其他内容”，在看 Feed（全部 / Releases / 被加星 / 被关注）时包含：
  - **Inbox 快捷入口**
...
- **Releases**：左侧展示按日报边界分组的 Release 信息流（Feed）；仅在日组切换处显示弱化日期分隔线，首个可见日组不显示前置分隔，右侧展示「Inbox 快捷入口」。
- **被加星**：左侧仅展示“谁给我的个人仓库加星”的社交活动卡片，按同一日边界分组；右侧展示「Inbox 快捷入口」。
- **被关注**：左侧仅展示“谁关注了我”的社交活动卡片，按同一日边界分组；右侧展示「Inbox 快捷入口」。
...
- `全部` tab 的历史组折叠规则：若该日组有对应真实日报，则默认显示日报卡片；点击卡头右侧的「Releases」后，当前组切换为“日期分界 + 原始 Release 列表”，并在分界右侧显示「日报」按钮用于切回日报视图。
