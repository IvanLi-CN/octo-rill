...
- 用户在非 scoped 的全局 `全部` tab 点击历史日组分界右侧的“生成日报”后，按钮进入 spinning 状态，组内容切换为占位日报卡片；生成成功后占位内容被真实日报替换。
- 用户在非 scoped 的全局 `全部` tab 点击历史日组分界右侧的“生成日报”后，若请求失败，当前日组会原位显示错误面板与“重试日报”；再次触发成功后，错误面板被真实日报替换。
...
- Given 历史日组没有对应日报
  When 用户点击“生成日报”且请求失败
  Then 当前日组原位显示错误说明与“重试日报”，而不是只出现短暂全局提示。

- Given 用户在某个历史日组中切换日报或 releases 视图
  When 切换完成
  Then 只影响当前日组，不会改动其他历史日组的显示状态。

- Given 任意 scoped focus 页存在历史动态，且全局缓存中有覆盖这些动态的日报
  When scoped `全部` 页面渲染完成
  Then 页面保留日期分隔与全部原始 scope 条目，不显示日报内容，也不按日报覆盖关系隐藏条目。

## 实现前置条件（Definition of Ready / Preconditions）

- 已确认 Dashboard 与 Storybook 均存在单独的 feed 展示入口。
- 已确认 `docs/specs/` 为本仓库规格根目录。
- 已确认当前需求属于 UI-affecting，需补 Storybook 与视觉证据。
...
- `全部` tab 历史日组默认折叠为完整日报卡：卡头左侧提供 icon-only `去日报`，右侧保留 `复制`，正文通过 brief detail 懒加载后展示完整内容
