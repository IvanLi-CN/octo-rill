...
- The manifest contract is aligned with Chromium desktop and Android Chrome/WebAPK update semantics: stable identity preserves the installed app, while a changed icon URL signals the new icon. The V1-to-V2 regression keeps approved icon pixels unchanged while proving the changed hashed URL and fresh bytes are retrievable. Apple Web Clips remain explicitly limited to user-controlled device-owned state.
