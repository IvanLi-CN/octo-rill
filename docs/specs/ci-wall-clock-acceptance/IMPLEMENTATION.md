...
- The Python driver resolves and freezes `main` and candidate SHAs before serial dispatch, verifies strict ancestry and the restricted file delta, downloads each run's result artifact, and writes auditable run/job records and E2E statistics.
...
- Acceptance statistics use the `Frontend E2E` job interval and compare test totals, final failures, and retry totals between each control/candidate pair. Candidate E2E P90 is capped at 420 seconds.

## Remaining Gaps

- Controlled GitHub-hosted acceptance must be rerun for the candidate head after CI convergence; its result is the release evidence for the 10-pair contract.
