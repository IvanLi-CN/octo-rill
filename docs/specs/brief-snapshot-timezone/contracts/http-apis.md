...
- 普通用户读取只返回服务端判定为已完成的日报快照；自然日窗口未关闭的快照不会出现在 Dashboard、侧栏、日报列表或日报详情中。
...
- 响应只包含已完成且对普通用户可见的快照。已结束窗口前写入的规范化快照必须在重新生成后才重新出现；缺失窗口元数据的历史快照按用户有效日期窗口兼容判断。
...
- 目标自然日窗口尚未关闭时，同步和异步请求均在执行生成或入队前返回 HTTP `409`：

```json
{
  "error": {
    "code": "daily_brief_window_open",
    "message": "日报对应的自然日尚未结束，请在次日 00:00 后生成"
  }
}
```
