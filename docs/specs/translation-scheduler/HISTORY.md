...
- 采集记录列表以来源时间而非发现时间筛选和排序，保留历史 `detected_at = NULL`；按记录级总尝试次数支持 `0..10` 及不限上限筛选，零次记录也参与总数和分页。
- 全局工作身份、用户请求关联、结果投影、旧事实展示与切换兼容性已移交给[全局翻译与润色工作模型](../global-translation-and-polish/SPEC.md)；本主题保留调度、批处理、租约、恢复与尝试审计职责。
