...
- Build contract coverage checks the generated manifest identity, content-hashed icon URLs and bytes, shortcuts, screenshots, PNG icon dimensions, precache URL safety, and Service Worker metadata cache-bypass guards.
...
- The manifest contract is aligned with Chromium desktop and Android Chrome/WebAPK update semantics: stable identity preserves the installed app, while a changed icon URL signals the new icon. Apple Web Clips remain explicitly limited to user-controlled device-owned state.
