...
- Release helper scripts:
  - `.github/scripts/release-intent.sh` 需要接受 `RELEASE_HEAD_SHA` 作为 push/backfill 的统一输入。
  - `.github/scripts/compute-version.sh` 需要在目标提交已有 release tag 时复用既有版本/tag，保证 rerun/backfill 幂等。
