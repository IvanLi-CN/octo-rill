...
- 首次进入 `/admin/jobs` 仍允许阻塞式加载；首载完成后，手动刷新、SSE 自动刷新，以及筛选触发的后续列表复载都必须保留现有内容。
- 为三块内容增加轻量刷新提示，避免整块内容消失。
- 后台刷新期间保留的旧列表进入只读状态，避免对即将失效的旧行继续触发详情/重试/取消等交互。
- 消除组件自身重复首载请求，保留一套清晰的初始化加载来源。
- 补齐 e2e 回归，覆盖 realtime / scheduled / llm 三块在手动刷新、SSE 刷新、筛选切换与重叠请求下的不闪烁场景。
...
- Given `实时异步任务` 或 `LLM 调度` 已经完成首载
  When 管理员切换状态筛选并触发后续列表复载
  Then 旧列表在请求完成前持续可见，仅显示轻量刷新提示；新结果返回后再切换到最新筛选内容。

- Given 某个列表正处于后台刷新中且仍显示旧行
  When 管理员尝试点击这些旧行的详情/重试/取消入口
  Then 这些入口处于禁用状态，直到最新结果返回后再恢复交互。
...
- E2E tests: 扩展 `web/e2e/admin-jobs.spec.ts`，覆盖 realtime / scheduled / llm 刷新期间保留已渲染内容、首次未加载完成时仍保持阻塞 loader、筛选切换期间不闪烁，以及重叠请求失败路径不会冒出过期错误横幅。
...
- 为 overview / realtime / scheduled / llm status / llm calls 建立显式加载阶段：仅首载继续使用阻塞式 loading；首载完成后，手动刷新、SSE 与筛选触发的后续列表复载统一走后台刷新，保留当前内容直到最新结果返回。
...
- 风险：背景刷新期间保留旧列表，会带来短时间的“旧结果仍可见”窗口；当前扩展到首载之后的筛选复载，同样依赖请求序号保护避免旧响应覆盖新结果。
- 风险：SSE 与手动刷新重叠时仍可能产生并发请求，需要与现有节流/排队逻辑一起保持最新请求优先。
- 假设：管理员接受“后台刷新保留旧数据”的交互优先级高于“立刻清空显示新筛选的空态”；筛选结果以请求完成时再原子切换。
