...
- **全部（聚合）**：按日报边界分组展示 Release 信息流；当前日组直接显示原始 Release 卡片，历史日组优先折叠为对应日报，并允许单独展开查看该日原始 Release。
- **Releases**：左侧展示按日报边界分组的 Release 信息流（Feed）；每个日组都有弱化日期分隔线，右侧展示「Inbox 快捷入口」。
...
- 组头样式：每个日组前使用弱化分隔线显示日期与当日 Release 数，用于帮助用户快速分辨“今天”与历史日组。
- `全部` tab 的历史组折叠规则：若该日组有对应真实日报，则优先显示日报；若无日报，则直接回退为原始 Release 卡片流。
