...
- Completed governance-cycle member retention must keep reconciliation and terminal queries scoped to active cycles. Prune completed members in a best-effort coordinator lane with a maximum 500-row delete transaction, retaining the cycle summary for reporting.
- When a retention budget must hold across restarts or concurrent runtime owners, persist its single shared window and deleted-row counter in SQLite and update it in the same `BEGIN IMMEDIATE` transaction as the bounded delete. `repo_refresh_governance_retention` uses a 50,000-row-per-minute budget; unavailable permits and SQLite busy are observable skips rather than failures.
...
- For very large retention tables such as `repo_release_watchers`, best-effort cleanup also needs sufficiently large delete batches and explicit completion telemetry. Tiny fixed batches can leave permanent backlog even when the writer contract is correct, which then amplifies unrelated admin and sync queries during incident diagnosis.
- Admin-side observability queries must avoid wrapping indexed RFC3339 timestamps in `datetime()` / `julianday()` when a straight lexical range on canonical UTC strings is sufficient. On SQLite this often determines whether the planner can reuse the intended index or falls back to a full scan / temp sort.
