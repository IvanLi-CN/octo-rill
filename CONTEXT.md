...
**无感数据迁移**:
一次 durable state 演进在用户原有可读或可提交的内容处理路径上不产生迁移专属的维护错误；已接受的工作和已发布的结果投影保持可追踪与可读取。它不是单纯的异步回填，也不等同于替换运行中的程序。
_Avoid_: `rollback_freeze`, 迁移维护窗口, 停机切换

**迁移操作**:
一个 durable state 变更中的独立持久化步骤，限定为结构变更、当前数据操作或历史数据回填之一。它具有自己的顺序、进度、暂停点和恢复结果。
_Avoid_: 整体 migration, 隐式启动任务

**迁移接纳**:
在数据演进期间将一次有效内容处理提交原子地关联到唯一权威工作项的行为。它保持原提交接口的排队、活动或结果语义，而不向调用者暴露迁移模式。
_Avoid_: 切换冻结, 双写提交, 迁移错误
