# Release reaction 反馈图标轻量收敛

## 背景 / 问题陈述

`release-reaction-bubble-polish` 已经把 Dashboard release card 的 reaction footer 收敛为本地 SVG、圆形按钮与外置 badge，但当前按钮本体、图标和 badge 仍然偏大，在正文区下方的视觉存在感过强，容易抢走 release 内容本身的注意力。
...
- badge 仍定位在按钮右上方偏外位置，但偏移量比 `release-reaction-bubble-polish` 版本更保守。
...
- Given release card 渲染带 reaction 的 footer
  When UI 显示完成
  Then 反馈按钮肉眼可见比 `release-reaction-bubble-polish` 版本更紧凑，但仍保持清晰可点按。
