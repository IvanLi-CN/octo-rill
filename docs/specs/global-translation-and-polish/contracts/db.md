...
`content_current_result_projections.identity_id` references the unique registry row for that full identity. The membership table carries that full identity key and uses composite foreign keys to require it to match both the registry row and retained work row. The projection's `work_item_id` must resolve through membership for that exact identity. Its optional `active_work_item_id` may have a newer `source_hash`, but must match the identity's resource, pipeline, variant, target language and protocol version; database triggers reject links outside that scope. Its optional `source_projection_id` must reference model-specific projection history with the same complete identity, including `source_hash`; database triggers reject mismatched provenance. The existing `content_result_projections` table remains model-specific history during the compatibility stage. A projection is a cache hit only when its registered `source_hash` matches the requested work identity. Retained model-profile or work-level configuration-fingerprint fields are historical provenance only; they must not affect identity, current-result lookup or attempt configuration. Do not infer or fabricate per-attempt snapshots from old work-level fingerprints.
...
- `blocked_config -> queued` is scheduler-owned and event-driven: a relevant global model-configuration update or runtime configuration reload triggers revalidation, and only a currently valid configuration may requeue the work. Admission and manual retry read persisted routes inside their work-writing SQLite transaction so the route update and work status have one order; when the update follows admission, its recovery scan observes any blocked row committed by that admission. Recovery requeues at most one item per identity and skips identities that already have queued, running, or deferred-provider work, so a full ineligible batch cannot pin the scan. Invalid configuration causes no provider call and no timer-based retry. The next attempt snapshots the new current configuration.
...
- Provider call success and business output success are separate facts. An `output_contract_invalid` failure may link a provider call whose provider status is `succeeded`, while the attempt, work item and batch remain failed and publish no projection. `output_truncated` uses the same failed-work boundary.
