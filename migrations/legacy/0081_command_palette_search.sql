...
-- Idempotent backfill from the current local caches. Existing rows are refreshed,
-- while the unique key prevents duplicate canonical documents on reruns.
INSERT INTO search_documents (
  id, user_id, resource_type, resource_id, repo_id, repo_full_name, owner_login,
  title, body, source_time, target_path, target_url, created_at, updated_at
)
SELECT
  'release:' || r.release_id,
  NULL,
  'release',
  CAST(r.release_id AS TEXT),
  r.repo_id,
  COALESCE((SELECT rwi.repo_full_name FROM repo_release_work_items rwi WHERE rwi.repo_id = r.repo_id LIMIT 1), (SELECT vr.full_name FROM user_release_visible_repos vr WHERE vr.repo_id = r.repo_id LIMIT 1), (SELECT ura.repo_full_name FROM user_repo_associations ura WHERE ura.repo_id = r.repo_id LIMIT 1), (SELECT sr.full_name FROM starred_repos sr WHERE sr.repo_id = r.repo_id LIMIT 1)),
  COALESCE((SELECT CASE WHEN instr(rwi.repo_full_name, '/') > 0 THEN substr(rwi.repo_full_name, 1, instr(rwi.repo_full_name, '/') - 1) ELSE rwi.repo_full_name END FROM repo_release_work_items rwi WHERE rwi.repo_id = r.repo_id LIMIT 1), (SELECT vr.owner_login FROM user_release_visible_repos vr WHERE vr.repo_id = r.repo_id LIMIT 1), (SELECT ura.owner_login FROM user_repo_associations ura WHERE ura.repo_id = r.repo_id LIMIT 1), (SELECT sr.owner_login FROM starred_repos sr WHERE sr.repo_id = r.repo_id LIMIT 1)),
  COALESCE(r.name, r.tag_name),
  r.body,
  COALESCE(r.published_at, r.created_at, r.updated_at),
  CASE WHEN COALESCE((SELECT rwi.repo_full_name FROM repo_release_work_items rwi WHERE rwi.repo_id = r.repo_id LIMIT 1), (SELECT vr.full_name FROM user_release_visible_repos vr WHERE vr.repo_id = r.repo_id LIMIT 1), (SELECT ura.repo_full_name FROM user_repo_associations ura WHERE ura.repo_id = r.repo_id LIMIT 1), (SELECT sr.full_name FROM starred_repos sr WHERE sr.repo_id = r.repo_id LIMIT 1)) IS NOT NULL
       THEN '/' || COALESCE((SELECT rwi.repo_full_name FROM repo_release_work_items rwi WHERE rwi.repo_id = r.repo_id LIMIT 1), (SELECT vr.full_name FROM user_release_visible_repos vr WHERE vr.repo_id = r.repo_id LIMIT 1), (SELECT ura.repo_full_name FROM user_repo_associations ura WHERE ura.repo_id = r.repo_id LIMIT 1), (SELECT sr.full_name FROM starred_repos sr WHERE sr.repo_id = r.repo_id LIMIT 1)) || '/releases/tag/' || r.tag_name END,
  r.html_url,
  r.updated_at,
  r.updated_at
...
INSERT INTO search_documents (
  id, user_id, resource_type, resource_id, repo_id, repo_full_name, owner_login,
  title, body, source_time, unread, target_path, target_url, created_at, updated_at
)
SELECT
  'announcement:' || e.id,
  e.user_id,
  'announcement',
  CASE WHEN e.repo_full_name IS NOT NULL AND e.discussion_number IS NOT NULL
       THEN lower(e.repo_full_name) || '#' || e.discussion_number
       ELSE e.id END,
  e.repo_id,
  e.repo_full_name,
  CASE WHEN instr(COALESCE(e.repo_full_name, ''), '/') > 0
       THEN substr(e.repo_full_name, 1, instr(e.repo_full_name, '/') - 1)
       ELSE e.repo_full_name END,
  e.title,
  e.body,
  e.occurred_at,
  NULL,
  CASE WHEN e.repo_full_name IS NOT NULL AND e.discussion_number IS NOT NULL
       THEN '/' || e.repo_full_name || '/discussions/' || e.discussion_number END,
  e.html_url,
  e.created_at,
  e.updated_at
...
INSERT INTO search_documents (
  id, user_id, resource_type, resource_id, repo_full_name, owner_login,
  title, body, source_time, unread, target_url, created_at, updated_at
)
SELECT
  'notification:' || n.user_id || ':' || n.thread_id,
  n.user_id,
  'notification',
  n.thread_id,
  n.repo_full_name,
  CASE WHEN instr(COALESCE(n.repo_full_name, ''), '/') > 0
       THEN substr(n.repo_full_name, 1, instr(n.repo_full_name, '/') - 1)
       ELSE n.repo_full_name END,
  n.subject_title,
  COALESCE(n.subject_type, '') || ' ' || COALESCE(n.reason, ''),
  n.updated_at,
  n.unread,
  n.html_url,
  COALESCE(n.updated_at, CURRENT_TIMESTAMP),
  COALESCE(n.updated_at, CURRENT_TIMESTAMP)
...
INSERT INTO search_documents (
  id, user_id, resource_type, resource_id, title, body, source_time,
...
SELECT
  ...
  '日报 ' || b.date,
  b.content_markdown,
  COALESCE(b.window_end_utc, b.created_at),
  '/briefs?brief=' || b.id,
  b.created_at,
  b.updated_at
FROM briefs b
...
SELECT
  'repository:' || ura.user_id || ':' || ura.repo_full_name_lower,
  ura.user_id,
  'repository',
  ura.repo_full_name_lower,
  ura.repo_id,
  ura.repo_full_name,
  ura.owner_login,
  ura.repo_name,
  ura.description,
  ura.updated_at,
  '/focus/repo/' || ura.owner_login || '/' || ura.repo_name,
  ura.html_url,
  ura.created_at,
  ura.updated_at
...
SELECT
  'repository:' || sr.user_id || ':' || lower(sr.full_name),
  sr.user_id,
  'repository',
  lower(sr.full_name),
  sr.repo_id,
  ...
  sr.description,
  sr.updated_at,
  '/focus/repo/' || sr.owner_login || '/' || sr.name,
  sr.html_url,
  sr.updated_at,
  sr.updated_at
...
-- Fold already-cached legacy translation/polishing rows into a user-scoped
-- overlay before the initial FTS build. New writes are handled by triggers.
INSERT INTO search_document_user_lanes (
  document_id, user_id, translated_text, smart_text, updated_at
)
SELECT
  d.id,
  t.user_id,
...
JOIN search_documents d
  ON ((d.resource_type = 'release' AND lower(t.entity_type) LIKE 'release%' AND d.resource_id = t.entity_id)
   OR (d.resource_type = 'announcement' AND lower(t.entity_type) LIKE 'announcement%' AND d.resource_id = lower(t.entity_id))
   OR (d.resource_type = 'notification' AND lower(t.entity_type) LIKE 'notification%' AND d.resource_id = t.entity_id))
WHERE lower(t.entity_type) NOT LIKE '%smart%'
  AND t.lang = 'zh-CN'
  ...
  AND NOT EXISTS (
    ...
    WHERE newer.user_id = t.user_id
      ...
      AND ((lower(t.entity_type) LIKE 'release%' AND lower(newer.entity_type) LIKE 'release%')
        OR (lower(t.entity_type) LIKE 'announcement%' AND lower(newer.entity_type) LIKE 'announcement%')
        OR (lower(t.entity_type) LIKE 'notification%' AND lower(newer.entity_type) LIKE 'notification%'))
      AND (newer.updated_at > t.updated_at OR (newer.updated_at = t.updated_at AND newer.id > t.id))
  )
ON CONFLICT(document_id, user_id) DO UPDATE SET
  translated_text = excluded.translated_text,
  smart_text = search_document_user_lanes.smart_text,
...
JOIN search_documents d
  ON ((d.resource_type = 'release' AND lower(t.entity_type) LIKE 'release%' AND d.resource_id = t.entity_id)
   OR (d.resource_type = 'announcement' AND lower(t.entity_type) LIKE 'announcement%' AND d.resource_id = lower(t.entity_id))
   OR (d.resource_type = 'notification' AND lower(t.entity_type) LIKE 'notification%' AND d.resource_id = t.entity_id))
WHERE lower(t.entity_type) LIKE '%smart%'
  AND t.lang = 'zh-CN'
  AND t.status IN ('ready', 'disabled', 'missing')
  AND (t.title IS NOT NULL OR t.summary IS NOT NULL)
  AND NOT EXISTS (
...
ON CONFLICT(document_id, user_id) DO UPDATE SET
  translated_text = search_document_user_lanes.translated_text,
  smart_text = excluded.smart_text,
  updated_at = MAX(search_document_user_lanes.updated_at, excluded.updated_at);

UPDATE search_documents
SET translated_text = (
      SELECT trim(COALESCE(json_extract(p.payload_json, '$.title'), json_extract(p.payload_json, '$.title_zh'), '') || ' ' || COALESCE(json_extract(p.payload_json, '$.summary'), json_extract(p.payload_json, '$.body_md'), ''))
      FROM content_result_projections p
      WHERE p.canonical_resource_type = search_documents.resource_type
        AND p.canonical_resource_id = search_documents.resource_id
        AND p.pipeline = 'translation'
        AND p.target_lang = 'zh-CN'
      ORDER BY CASE WHEN EXISTS (SELECT 1 FROM content_work_items w WHERE w.id = p.active_work_item_id AND w.status = 'ready') THEN 0 ELSE 1 END,
    ...
    smart_text = (
      ...
      WHERE p.canonical_resource_type = search_documents.resource_type
        AND p.canonical_resource_id = search_documents.resource_id
        AND p.pipeline = 'polishing'
        AND p.target_lang = 'zh-CN'
      ORDER BY CASE WHEN EXISTS (SELECT 1 FROM content_work_items w WHERE w.id = p.active_work_item_id AND w.status = 'ready') THEN 0 ELSE 1 END,
               p.updated_at DESC, p.id DESC
      LIMIT 1
    )
WHERE resource_type IN ('release', 'announcement', 'notification');

DELETE FROM search_documents_fts;

INSERT INTO search_documents_fts (doc_id, title, body, repo_full_name, translated_text, smart_text)
SELECT id, COALESCE(title, ''), COALESCE(body, ''), COALESCE(repo_full_name, ''),
       COALESCE(translated_text, ''), COALESCE(smart_text, '')
FROM search_documents;

DELETE FROM search_document_user_lanes_fts;
...
CREATE TRIGGER IF NOT EXISTS search_repo_releases_ai AFTER INSERT ON repo_releases BEGIN
  ...
  ON CONFLICT(id) DO UPDATE SET repo_id=excluded.repo_id, repo_full_name=excluded.repo_full_name, owner_login=excluded.owner_login, title=excluded.title, body=excluded.body, source_time=excluded.source_time, target_url=excluded.target_url, updated_at=excluded.updated_at;
